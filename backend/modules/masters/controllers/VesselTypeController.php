...
class VesselTypeController extends Controller {
    ...
    public function actionIndex($id = NULL) {
        ...
        if ($model->load(Yii::$app->request->post()) && Yii::$app->SetValues->Attributes($model) && $model->validate()) {
            if ($model->save()) {
                if (!empty($id)) {
                    Yii::$app->getSession()->setFlash('success', 'Vessel Type Details updated Successfully');
                } else {
                    Yii::$app->getSession()->setFlash('success', 'Vessel Type Details created Successfully');
                }
