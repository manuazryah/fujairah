...
class SupplierFundingController extends Controller {
    ...
    public function actionSaveSupplierPrice() {
        ...
        $this->UpdateSupplierPrice($arr);
        $id = $_POST['app_id'];
        return $this->redirect(['add', 'id' => $id]);
    }

    public function UpdateSupplierPrice($arr) {
        foreach ($arr as $key => $value) {
            $supplier_model = SupplierFunding::findOne(['close_estimate_id' => $key]);
            if (isset($value['invoice_number'])) {
                $supplier_model->invoice_number = $value['invoice_number'];
            }
            if (isset($value['invoice_date'])) {
                $supplier_model->invoice_date = $value['invoice_date'];
            }
            if ($supplier_model->amount_debit == '') {
                $supplier_model->amount_debit = $value['amount_debit'];
    ...
    public function AddSupplierPrice($supplier_model, $value) {
        ...
        $model_supplier->invoice_number = $supplier_model->invoice_number;
        $model_supplier->invoice_date = $supplier_model->invoice_date;
