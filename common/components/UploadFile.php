...
class UploadFile extends Component {
    ...
    public function UploadSingle($files, $model, $paths) {
        if ($files != '' && $model != '') {
            ...
            $name = $this->fileExists($path, str_replace(' ', '_', strtolower($files->baseName)) . '.' . $files->extension, $files, 1);
    ...
    public function ListFile($id, $type) {
        ...
        foreach (glob("{$path}/*") as $file) {
            if ($file != '') {
                ...
                $attach .= '<a target="_blank" href="' . Yii::$app->homeUrl . Yii::$app->params['mainPath'] . '/' . Yii::$app->params['appointmentPath'] . '/' . $folder . '/' . $id . '/' . $type . '/' . end($arry) . '" title="' . end($arry) . '">' . substr(end($arry), 0, 25) . '</a>&nbsp;&nbsp;<a href="' . Yii::$app->homeUrl . Yii::$app->params['appointmentPath'] . '/' . $type . '/' . 'remove?path=' . $path . '"><i class="fa fa-remove"></i></a>&nbsp;&nbsp;|&nbsp;&nbsp;';
            }
        }
        // echo $attach;exit;
        return $attach;
